...
import {onlineStep,applyRemoteControls,publishSnapshot,receiveSnapshot} from './online-combat.js';
...
const match=new Match(()=>{fighters=app.selected.map((type,i)=>new Fighter({x:i?950:250,facing:i?-1:1,type,side:i?'p2':'p1',label:'P'+(i+1)}));effects=[];aiCooldown=0;controls.keys.clear();hud.mount(fighters);},side=>{app.stats.matches++;app.stats[side?'p2Wins':'p1Wins']++;app.saveStats();publishSnapshot(app,fighters,match,50);app.show('victory',{side});});
app.startMatch=()=>{app.show(STATES.BATTLE);match.reset();};
app.applyOnlineSnapshot=data=>{if(app.state==='battle')receiveSnapshot(app,fighters,match,data);};
hud.hide();
...
function update(ms){if(app.state!=='battle')return;if(onlineStep(app,fighters,match,ms))return;try{match.step(ms);if(match.phase!=='active')return;if(!match.remaining){const [p1,p2]=fighters;match.award(p1.hp===p2.hp?-1:p1.hp>p2.hp?0:1);return;}controls.apply(fighters[0],'p1',0,audio);if(app.mode==='AI')ai();else if(app.mode==='ONLINE')applyRemoteControls(app,fighters[1],audio);else controls.apply(fighters[1],'p2',1,audio);fighters[0].update(fighters[1],ms);if(match.phase!=='active')return;fighters[1].update(fighters[0],ms);effects=effects.filter(e=>{e.y-=.5;return --e.life>0;});}finally{publishSnapshot(app,fighters,match,ms);}}
...
window.addEventListener('pagehide',()=>{app.onlineDispose?.();app.net?.disconnect();loop.stop();audio.channels.forEach(channel=>channel.pause());});
boot();
