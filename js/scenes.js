...
export class Scenes{
  ...
  menu(name){const app=this.app,t=app.t,host=this.host;host.classList.add('menu');host.append(el('h1',t(name==='root'?'menu':name)));const list=group('menu-list');
    const menus={root:['local','multi','help','profile','options','exit'],local:['arcade','versus','campaign','tournaments'],multi:['quick','ranked','room'],help:['guide','controls','credits']},blocked=new Set(['campaign','tournaments','quick','ranked']);
